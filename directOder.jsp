...
<body>
<center>
<img src="./images/order_01.gif" border=0>
<%
...
<font style="font-size:10pt;font-family:맑은 고딕">
		영화 예매를 위해서는 로그인이 필요합니다! <br><br>	<a href="login.jsp" ><img src="./images/login.gif" border=0></a>
</font>
</center>
<%
}
else
{
 try { // (1)  (34행~151행):  tempcart 테이블에 즉시구매할 상품레코드 저장후, tempcart 내역을 그대로 보여주기 
	...
	//   방지하기 위하여, "즉시구매하기"를 수행하기 직전에 명시적으로 tempcart 테이블을 비워주도록 처리함.
	 String jsql5 = "delete from reservation where ctNo=?";    
...
<table border=1 color style="font-size:10pt;font-family:맑은 고딕">
<tr>       
   <td> <img src="./images/order01_03.gif" border=0><p align="center"><font size="2" color="#ECFAE4"><strong>번호</strong></font></td>       
   <td bgcolor="#002C57"  width = 120 height="30" align="center"><p><font  size="2" color="#ECFAE4"><strong>영화명</strong></font></td>       
   <td bgcolor="#002C57"  width = 120 height="30" align="center"><p><font  size="2" color="#ECFAE4"><strong>가격(원)</strong></font></td>       
   <td bgcolor="#002C57"  width = 120 height="30" align="center"><p><font  size="2" color="#ECFAE4"><strong>주문수량(장)</strong></font></td>     
   <td bgcolor="#002C57"  width = 120 height="30" align="center" ><p><font size="2" color="#ECFAE4"><strong>총액(원)</strong></font></td>  
...
//         그런 다음, 추출해 낸 각 필드들 및 계산된 결과값들을 웹브라우저상에 출력해 준다. 
		...
		String jsql2 = "select no, price, qty from reservation where ctNo = ?";   //  tempcart 테이블을 사용함에 유의!
	...
	//	while(rs2.next()) 
  	//	{			                   
		...
		String prdNo = rs2.getString("no");	    //  tempcart 테이블로부터 상품번호 추출
   		int ctQty = rs2.getInt("qty");	                    //  tempcart 테이블로부터 주문수량 추출 

   		String jsql3 = "select prdName, prdPrice from movie where no = ?";
