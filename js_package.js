...
function checkValue()             // 회원가입폼에서 미입력한 항목이 있을 경우, 경고메시지를 보여주는 함수
{
    ...
    {
		alert("확인비밀번호를 입력해 주세요!");
		newMem.password2.focus();
		return;
    }
	if(newMem.password1.value !=newMem.password2.value){
	alert("비밀번호가 다릅니다.");  //  문자열이 포함된 경고창을 실행시킴
		newMem.password2.focus();					//  작업의 포커스(커서)를 ID를 입력하는 텍스트필드로 이동시킴  
		return;                         //  함수의 실행을 종료함 ( "return false; " 라고 사용해도 동일함)
	}
...
function upcheckValue()             // 회원수정폼에서 미입력한 항목이 있을 경우, 경고메시지를 보여주는 함수
{
	if(upMem.id.value == "")
    {
		alert("ID를 입력해 주세요!");
...
function allupcheckValue()             // 회원수정폼에서 미입력한 항목이 있을 경우, 경고메시지를 보여주는 함수
{
